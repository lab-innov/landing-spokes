...
TEMPLATE_FILE="${REPO_ROOT}/infra/ifiscaf-spoke/main.bicep"

PARAM_FILE="${1:-}"
ACTION="${2:-what-if}"
DEPLOYMENT_NAME="${3:-ifiscaf-secure-spoke-$(date +%Y%m%d%H%M%S)}"
...
"${SCRIPT_DIR}/preflight-ifiscaf-spoke.sh" "${PARAM_FILE}" "${ACTION}"
