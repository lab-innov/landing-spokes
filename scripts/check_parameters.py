...
def check(p):
    ...
    for key in ('apiClientPrefixes','operatorPrefixes','monitorPrefixes','functionPrivateIps','cosmosPrivateIps','processorPrefixes','processorDataPrivateIps'):
        values=p.get(key,[])
        for value in values:
            try:
                n=ipaddress.IPv4Network(value,strict=True)
                require(any(n.subnet_of(v) for v in PRIVATE),f'{key}: red privada requerida.')
                if key in ('functionPrivateIps','cosmosPrivateIps','processorDataPrivateIps'):require(n.prefixlen==32 and '/' not in value,f'{key}: IP individual sin máscara requerida.')
