"""Contratos MOP, incluidos casos de rechazo y límites de inventario."""
...
def foundation():return dict(tags={'iniciativa':'MOP','DataClassification':'Interna','OpsDept':'DTI','UserDept':'Área funcional'},functionAppName='func-mop-new',planName='plan-mop-new',storageAccountName='stmopnew',businessStorageAccountName='stmopdocs',cosmosAccountName='cosmos-mop-new',openAiAccountName='oai-mop-new',documentIntelligenceAccountName='di-mop-new',vnetAddressPrefixes=['10.181.0.0/24'],subnetPrefixes={'privateEndpoints':'10.181.0.0/27','functionsIntegration':'10.181.0.64/26'},routeTableResourceId=rid('Microsoft.Network/routeTables'),logAnalyticsWorkspaceResourceId=rid('Microsoft.OperationalInsights/workspaces'),privateDnsZoneResourceIds={service:f'/subscriptions/{SID}/resourceGroups/RG-PRIVATEDNS-PR/providers/Microsoft.Network/privateDnsZones/privatelink.{service}.example' for service in ('blob','queue','table','sites','cosmosSql','cognitiveServicesAccount')})
...
class Parameters(unittest.TestCase):
    ...
    def test_durable_cannot_be_disabled(self):
        self.assertTrue(checker.check(dict(foundation(),hostUsesDurableStorage=False)))
    ...
    def test_invalid_network_and_permissions(self):
        for key,value in [('allowedOrigins',['*']),('functionAppName','azfunc-premop-CR'),('functionPrivateIps',['10.181.0.1']),('routeTableResourceId','x'),('siemEventHubName','unpaired'),('extraEgress',[{'purpose':'functionsIntegration','destination':'0.0.0.0/0','ports':['*'],'justification':''}])]:
...
class Template(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.arm=json.loads(Path(os.environ['MOP_ARM']).read_text());cls.resources=[]
        def walk(t):
    ...
    def test_cosmos_schema_and_batch_scope(self):
        ...
        self.assertEqual(self.arm['resources']['serviceEndpoints']['copy']['count'],"[length(range(0, 4))]")
    ...
    def test_original_capacity_preserved(self):
        ...
        self.assertEqual(plan['sku']['name'],'B2');self.assertEqual(plan['sku']['capacity'],1)
        storage=next(r for r in self.resources if r['type']=='Microsoft.Storage/storageAccounts')
        self.assertEqual(storage['sku']['name'],"[parameters('storageSku')]");
        self.assertEqual(self.arm['resources']['businessStorage']['properties']['parameters']['storageSku']['value'],'Standard_RAGRS');self.assertFalse(storage['properties']['allowSharedKeyAccess'])

class Network(unittest.TestCase):
    profiles=json.loads((ROOT/'infra/network-rules.json').read_text())
    addresses={'functionsIntegration':['10.181.0.64/26'],'privateEndpoints':['10.181.0.0/27'],'function':['10.181.0.5'],'cosmos':['10.181.0.6'],'clients':['10.178.0.0/24'],'operators':['10.177.0.4'],'monitor':['10.179.1.4']}
