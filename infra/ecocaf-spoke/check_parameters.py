...
def check(p):
    ...
    for key in ('dnsServers','apiClientPrefixes','operatorPrefixes','monitorPrefixes','functionPrivateIps'):
        values=p.get(key,[])
        ...
        for value in values:
            try:
                ...
                if key in ('dnsServers','functionPrivateIps'):require(n.prefixlen==32,f'{key}: IP individual requerida.')
