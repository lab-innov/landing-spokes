...
def check(p):
    ...
    for key,old in [('functionAppName','azfun-poc-eco-caf-cr'),('planName','appsp-poc-eco-caf-cr'),('storageAccountName','rgpocecocafcraf57')]:
    ...
    require(set(zones)=={'blob','queue','table','sites'},'Definir las cuatro zonas DNS privadas de ECOCAF.')
    ...
    try:
        ...
        require(set(nets)=={'privateEndpoints','functionsIntegration'},'Definir las dos subredes ECOCAF.')
    ...
    for key in ('apiClientPrefixes','operatorPrefixes','monitorPrefixes','functionPrivateIps'):
        values=p.get(key,[])
        for value in values:
            try:
                n=ipaddress.IPv4Network(value,strict=True)
                require(any(n.subnet_of(v) for v in PRIVATE),f'{key}: red privada requerida.')
                if key=='functionPrivateIps':require(n.prefixlen==32,f'{key}: IP individual requerida.')
                if key=='functionPrivateIps' and 'privateEndpoints' in nets:
