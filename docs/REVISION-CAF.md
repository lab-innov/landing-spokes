...
- Los IDs de zonas DNS centrales deben confirmarse; el endpoint del recurso Foundry asocia las tres zonas requeridas (`cognitiveservices`, `openai` y `services.ai`). Plataforma debe publicar por separado el dominio interno ACA y el nombre web. Requiere AMPLS corporativo y certificado TLS en Key Vault.
